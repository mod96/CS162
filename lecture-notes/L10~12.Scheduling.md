...
The Banker's Algorithm is a resource allocation and deadlock avoidance algorithm that tests for safety by simulating the allocation for the predetermined maximum possible amounts of all resources, then makes an “s-state”(safe, unsafe) check to test for possible activities, before deciding whether allocation should be allowed to continue.
...
If `thread A`'s line1 -> `thread B`'s line2 comes, scheduler determines letting `y.Acquire()` would cause unsafe state, so make `thread B` to sleep. (If not using banker's algorithm, it would cause deadlock)

Below is simple illustration how banker's algorithm kinda works.
